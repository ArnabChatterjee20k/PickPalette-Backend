...
export default class PaletteDB {
  ...
  async updateDB(palettes: string[][]) {
    ...
    if (isUpdated) await this.updateCounter(filteredPalettes.length);
  }

  private async addNewPalettes(palettes: string[][]) {
    const paletteSheet = this.db.sheetsByTitle.Palette;
    console.log({ message: "Saving", palettes });
    try {
      await paletteSheet.addRows(palettes);
      return true;
    } catch (error) {
      console.log({ message: "Saving to the palette sheet", error });
      return false;
    }
  }

  private async updateCounter(numberOfPalettesUpdated: number) {
